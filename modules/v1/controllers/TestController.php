...
class TestController extends ActiveController
{
    ...
    public function actionUpdateVisibilityRanks()
    {
         $sql="
                SELECT *,RANK () OVER (PARTITION BY user_role_id ORDER BY points DESC,sales_year DESC) user_rank,ROW_NUMBER () OVER (PARTITION BY user_role_id  ORDER BY points DESC) FROM (SELECT DISTINCT vc.user_id, u.user_role_id, vc.sales_month, vc.sales_qty, vc.sales_year, vc.points AS points, vc.sales_week, vc.kpis_id
                FROM hnk_visibility_trans_cumulative AS vc
                LEFT JOIN users AS u ON (u.id = vc.user_id)
                WHERE vc.kpis_id = 4) AS a1 ORDER BY user_role_id ,a1.sales_year desc
         ";
         ...
         {
             $sql2="
                        UPDATE hnk_visibility_trans_cumulative
                        SET user_rank=".$val['row_number']."
                        WHERE user_id=".$val['user_id']." 
                              AND sales_month=".$val['sales_month'].
                              "AND sales_year=".$val['sales_year'].
                              "AND  kpis_id=4"
                              ;
